  ...
  public:
    ...
    std::unique_ptr<OpenMM::Force> generate() const noexcept override
    {
        const std::string potential_formula = fmt::format(
            "EXV{id}_epsilon * (sigma_sum)^12 * ((1/r)^12 - EXV{id}_cutoff_correction);"
            "sigma_sum = EXV{id}_sigma1 + EXV{id}_sigma2", fmt::arg("id", ffgen_id_));

        auto exv_ff = std::make_unique<OpenMM::CustomNonbondedForce>(potential_formula);

        exv_ff->addPerParticleParameter(fmt::format("EXV{}_sigma", ffgen_id_));
        exv_ff->addGlobalParameter(fmt::format("EXV{}_epsilon", ffgen_id_), eps_);
        ...
        exv_ff->addGlobalParameter(fmt::format("EXV{}_cutoff_correction", ffgen_id_), cutoff_correction);
