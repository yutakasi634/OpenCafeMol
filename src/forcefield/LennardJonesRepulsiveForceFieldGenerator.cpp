#include "LennardJonesRepulsiveForceFieldGenerator.hpp"
...
LennardJonesRepulsiveForceFieldGenerator::LennardJonesRepulsiveForceFieldGenerator(const double cutoff_ratio,
      ...
      use_periodic_(use_periodic), ffgen_id_(fmt::format("LJRP{}", ffid.gen()))
{
    log_assert(this->sigmas_.size() == this->epsilons_.size(),
        "LennardJonesRepulsive: sigma.size(={}) must be the same as epsilon.size(={})",
        sigmas_.size(), epsilons_.size());

    for(std::size_t i=0; i<sigmas_.size(); ++i)
    {
        log_assert(sigmas_.at(i).has_value() == epsilons_.at(i).has_value(),
            "LennardJonesRepulsive: if particle i(={}) has sigma, it should have epsilon too, and vice versa.", i);
    }
...
std::unique_ptr<OpenMM::Force> LennardJonesRepulsiveForceFieldGenerator::generate() const
{
